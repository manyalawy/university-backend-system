...
const assignInst = async (req, res) => {
  try {
    ...
    if (course.course_coordinator_id !== req.userid) {
      return res
        .status(400)
        .json({ msg: "You arenot the coordinator of this course" });
    }
    ...
    function funct(item) {
      try {
        if (item.id === id) {
          ...
          return res
            .status(400)
            .json({ msg: "Instructor already added to course" });
        }
      } catch (error) {
    ...
    if (!flagFound) {
      await courses.updateOne(
        { name: course_name },
        { $push: { instructors: courseInstructor } }
      );
  ...
  try {
    ...
    if (courseRemove.course_coordinator_id !== req.userid) {
      return res
        .status(400)
        .json({
          msg: "User is not the coordinator of the course to be removed from",
        });
    }
    if (courseAdd.course_coordinator_id !== req.userid) {
      return res
        .status(400)
        .json({
          msg: "User is not the coordinator of the course to be added to",
        });
    }
    ...
    if (!flagInRem) {
      return res
        .status(400)
        .json({ msg: "Instructor not in course to be removed from" });
    }
    //unassignInst
    let updatedCourse1;
    if (flagInRem) {
      await courses.updateOne(
        ...
        { $pull: { instructors: courseInstructor } }
      );
...
const unassignInst = async (req, res) => {
  try {
    ...
    if (course.course_coordinator_id !== req.userid) {
      return res
        .status(400)
        .json({ msg: "User is not the coordinator of this course" });
    }
    //unassignInst
    await courses.updateOne(
      { name: course_name },
...
const viewStaffAll = async (req, res) => {
  try {
    const staff = await courses.find(
      { course_coordinator_id: req.userid },
      "instructors"
    );
    if (!staff) {
  ...
  try {
    ...
    if (dayOff === null) {
      return res
        .status(400)
        .json({
          msg: "This instructor doesnot teach courses in this hod's department",
        });
    }
    insts.forEach(funct);
  ...
  try {
    const requestList = await requests.find({
      $and: [
        ...
        { $or: [{ type: /leave/ }, { type: "change day off" }] },
      ],
    });
    if (!requestList) {
      return res
        ...
        .json({
          msg:
            "No requests of type leave or change day off issued for this hod",
        });
    }
...
const acceptRequest = async (req, res) => {
  try {
    ...
    const mem = await academicMembers.updateOne(
      { _id: request.sender_id },
      { $pull: { missing_days: request.date_to_be_requested } }
    );
    if (request.type === "annual leave")
      await academicMembers.updateOne(
        { _id: request.sender_id },
        { $set: { annual_leave_balance: mem.annual_leave_balance - 1 } }
      );
    if (request.type === "accidental leave")
      await academicMembers.updateOne(
        { _id: request.sender_id },
        {
          $set: {
            accidential_leave_balance: mem.accidential_leave_balance - 1,
            annual_leave_balance: mem.annual_leave_balance - 1,
          },
        }
      );
    res.json(request);
...
const viewAssignments = async (req, res) => {
  try {
    const slots = await courses.find(
      { course_cooridinator_id: req.userid },
      "slots"
    );
    if (slots.length === 0) {
      return res
        .status(400)
        .json({
          msg: "No teaching assignments for courses of this coordinator",
        });
    }
    res.json(slots);
  } catch (error) {
