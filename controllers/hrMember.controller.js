...
const Add_course=async(req,res)=>{
    ...
    if(isValid_Did==true && isValid_Ci==true){
        ...
        if(!search_cc){
            return res.status(400).json({msg: "This academic member is not found"});
        }
...
const addUser = async (req, res) => {
    ...
    try {
      ...
      const loc = await location.findById(req.body.location_id).exec();
      if (!loc) {
        return res.json({ error: "Location does not exist" });
      }
      if (loc.loc_type != "Office") {
        return res.json({ error: "Location should be of type office" });
      }
      if (loc.max_capacity == loc.current_capacity) {
        return res.json({ error: "Room reached maximum capacity" });
      }
      loc.current_capacity = loc.current_capacity + 1;
      loc.save();
      const salt = await bcrypt.genSalt(10);
      const hashedPassword = await bcrypt.hash("123456", salt);
  
      if (req.body.role == "HR") {
        const user = await hrMembers.find({}).exec();
        let count = null;
        if (user.length == 0) {
          count = 0;
        }
        if (user.length > 0) {
          count = parseInt(user[user.length - 1].id.substring(3)) + 1;
        }
        ...
        const newUser = new hrMembers({
          ...
          office_loc: req.body.location_id,
      ...
      if (req.body.role != "HR") {
        const user = await academicMembers.find({}).exec();
